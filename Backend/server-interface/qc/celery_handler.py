...
from .modules.CleanupModule.CleanupModule import CleanupTask
# @@/CELERYQCBASETASKIMPORTS
...
@celery.task(base=CleanupTask, name='celery.qcProcSessionCleanupModule')
def qcProcSessionCleanupModule(name, sessionId, prevTime=None, slistIdx=0, batchSize=5) -> None:
    """
    Goes through the list of recordings for this session in the redis database
    located at: 'session/sessionId/recordings', containing a list of all current
    recordings of this session, in the backend continuing from
    ...
    result = qcProcSessionCleanupModule.processBatch(name, sessionId, indices)
    if result:
        # continue the task chain
        qcProcSessionCleanupModule.apply_async(
           args=[name, sessionId, curTime, newSListIdx, batchSize])
