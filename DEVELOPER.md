...
If running on a large batch of recordings, it might be better to reduce the timeout and work on a bigger batch size (modifiable in [`qc/celery_config.py`](https://github.com/Eyra-is/Eyra/blob/master/Backend/server-interface/qc/celery_config.py)). E.g. change `batch_size` to 500 and `session_timeout` to 5 seconds. Then you can skip requerying the sessions to avoid the timeout, and they should be done in big batches.  
In dire situations do a `redis-cli -n 1`->`flushdb` (or -n 0). And then restart Celery/apache.
